...
class builder
{
	 ...
	 */
	public function __construct(\phpbb\auth\auth $auth, \phpbb\language\language $language, \phpbb\request\request_interface $request, Container $phpbb_container, \phpbb\user $user, \blitze\content\services\fields $fields, \blitze\content\services\types $types, \blitze\content\services\form\form $form, $phpbb_root_path, $php_ext)
	{
		$this->auth = $auth;
	...
	public function init($forum_id, $mode, $save_draft)
	{
		if ($type = $this->types->get_forum_type($forum_id))
		{
			...
			$this->content_fields = $entity->get_content_fields();
